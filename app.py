...
I have the below code, looking to include all my portfolio into the code, can you add these tickers to the code,
...
Here is the updated Streamlit code with all the unique stock/ETF ticker symbols extracted from your Yahoo Finance portfolios added to the default_tickers list:
...
st.plotly_chart(fig, use_container_width=True)
Notes:
Deduplication: Duplicates across your various Yahoo Finance lists (like TSLA, NVDA, MCD, AAPL, UUUU, F) were deduplicated to keep the request clean.

Excluded Symbols: Non-standard ticker formats (like indices ^DJI, commodities SI=F, or non-tradable placeholder symbols) were omitted so yfinance doesn't throw errors when fetching data.
